...
#### 5. Keep the Market Capitalizations have the same length as stock returns
...
#### 7. Calculate the EBIT/Asset (ROA) rather than ROE as the indication of operating profitabilty
...
#### 3. Divide those comopanies based on their PE
...
#### 4. Divide those comopanies based on their OP
...
#### 5. Divide those comopanies based on their INV (investment growth rate)
...
#### 4. Sort stock returns based on their operating profitabiltiy (OP) from robust to weak
...
#### 6. Standardized Firm characteristisc
#### Standardization: re-scale time-series firm characteristics (ln_size, PE, OP, INV)

$$(x - mean(x)) / sd(x)$$
#### 6.1 The standardized natural logarithm of market capitalization
```{r}
Dates <- df_Size$Dates
...
count_nas
```
#### Standardized the log size of 18 portfolios
```{r}
