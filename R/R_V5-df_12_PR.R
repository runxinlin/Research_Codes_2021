...
#Calculate historical cumulative returns for campanies.
gap = 21
win = 250

df_MOM <- data.frame()
for (i in 1:(dim(df_return)[1]-win)){
  mom <- colSums(df_return[i:(win+i-gap-1), -c(1,2)], na.rm = T)
  df_MOM <- rbind(df_MOM,mom)
}
colnames(df_MOM) <- colnames(df_return)[-c(1,2)]
df_MOM<-cbind(Dates = df_return[(win+1):dim(df_return)[1],1],df_MOM)

df_MOM <- df_MOM[1:(Periods*Holding),]

holding_MOM_list <- vector(mode = "list", length = Periods)
for (x in 1:Periods){
  holding_MOM_list[[x]] <- df_MOM[(1+(x-1)*Holding):(x*Holding),]
}
...
hld_period_calcs1 <- function(a, b, weights =1, port_number){
  ...
  level = rollapply(level, FUN = prod, na.rm =T, width = 1:(Holding +1), align = "right")
...
df_FF4 <- portfolio_clean4 %>% select(Dates, MR_RF, SMB, HML, WML)
...
df_std_PB <- cbind(df_PB[,1],data.frame(t(df_std_PB)))
colnames(df_std_PB)<- colnames(df_PB)

df_std_MOM <-  apply(t(df_MOM[, -1]), 2, scale)
df_std_MOM <- cbind(df_MOM[,1],data.frame(t(df_std_MOM)))
colnames(df_std_MOM)<- colnames(df_MOM)

# apply(df_std_PB[,-1], 1, mean, na.rm =T)
...
hld_size_calcs <- function(a, b, c, port_number1, port_number2){
  ...
  row_mean = rollapply(port, FUN = mean, na.rm = TRUE, width = 1:(Holding +1), align = "right",by.column = F)
  return(row_mean)
}
...
df_12_PB <- data.frame(Dates = df_return$Dates[-c(1:(Lookback+Gap))][1:(Periods*Holding)],
                       ...
                       B_Growth = unlist(Map(hld_PB_calcs, holding_PB_list, HML, SMB,
                                             port_number1 = 3, port_number2 = 2)),
                       B_Winner = unlist(Map(hld_PB_calcs, holding_PB_list, WML, SMB,
                                             port_number1 = 3, port_number2 = 2)),
                       B_Neutral_MOM = unlist(Map(hld_PB_calcs, holding_PB_list, RMW, SMB,
                                                  port_number1 = 2, port_number2 = 2)),
                       B_Loser = unlist(Map(hld_PB_calcs, holding_PB_list, WML, SMB,
                                            port_number1 = 1, port_number2 = 2)),
                       ...
                       S_Growth = unlist(Map(hld_PB_calcs, holding_PB_list, HML, SMB,
                                             port_number1 = 3, port_number2 = 1)),
                       S_Winner = unlist(Map(hld_PB_calcs, holding_PB_list, WML, SMB,
                                             port_number1 = 3, port_number2 = 1)),
                       S_Neutral_MOM = unlist(Map(hld_PB_calcs, holding_PB_list, WML, SMB,
                                                  port_number1 = 2, port_number2 = 1)),
                       S_Loser = unlist(Map(hld_PB_calcs, holding_PB_list, WML, SMB,
                                            port_number1 = 1, port_number2 = 1))
)

count_nas <- sapply(df_12_PB, function(x){sum(is.na(x))})
count_nas

df_12_std_PB <-  apply(t(df_12_PB[, -1]), MARGIN = 2, scale)
df_12_std_PB <- cbind(Dates,data.frame(t(df_12_std_PB)))
colnames(df_12_std_PB) <- colnames(df_12_PR)

# apply(df_12_std_PB[,-1], 1, mean, na.rm =T)
...
df_12_MOM <- data.frame(Dates = df_return$Dates[-c(1:(Lookback+Gap))][1:(Periods*Holding)],
                       B_Value = unlist(Map(hld_MOM_calcs, holding_MOM_list, HML, SMB,
                                            port_number1 = 1, port_number2 = 2)),
                       B_Neutral_PE = unlist(Map(hld_MOM_calcs, holding_MOM_list, HML, SMB,
                                                 port_number1 = 2, port_number2 = 2)),
                       ...
                       S_Value = unlist(Map(hld_MOM_calcs, holding_MOM_list, HML, SMB,
                                            port_number1 = 1, port_number2 = 1)),
                       S_Neutral_PE = unlist(Map(hld_MOM_calcs, holding_MOM_list, HML, SMB,
                                                 port_number1 = 2, port_number2 = 1)),
