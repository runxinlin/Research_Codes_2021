...
top100_price <- read_csv("top100_price.csv")

####Sorting Data####
#Solution1
top100_labels <- colnames(top100_price)[-1]

labels <- vector(mode =  "character", length = length(top100_labels))
for(i in 1:length(top100_labels)){
  labels[i] <- sub("\\:.*", "", top100_labels[i])
}
for(i in 1:length(labels)){
  labels[i] <- paste(labels[i],"SJ", "Equity",sep = " ")
}
labels <- intersect(labels, colnames(df_Price))
top_100 <- labels

# df_Size_tmp <- df_Size[,c("Dates", top_100)]
...
#   as.data.frame()
# 
# df_Size_tmp <- df_Size_tmp[-dim(df_Size_tmp)[1],]
# df_Size_tmp <-df_Size_tmp %>% select(where(~n_distinct(.) > 1)) # remove columns with same values
...
#Solution3
# labels <- vector(mode = "list", length = dim(df_Size_tmp)[1])
# #
...
#   labels[i] = data.frame(names = names(order_size)[1:100]) # change the top range here
# }
# 
# top_100 <- as.character(na.omit(unique(unlist(labels))))

# truncate each dataframe
df_Price <- df_Price[,c("Dates",top_100)]
df_Size <- df_Size[,c("Dates",top_100)]
df_PE <- df_PE[,c("Dates",top_100)]
df_EBIT <- df_EBIT[,c("Dates",top_100)]
df_Asset <- df_Asset[,c("Dates",top_100)]
...
hld_period_calcs1 <- function(a, b, weights =1, port_number){
  ...
  #delete the companies which the return rate is full with 0s or contain any NAs
...
lapply(df_FF4[,-1], function (x) {(exp(mean(x, na.rm = T)*252))-1})[4]
...
NWtest(df_FF4$WML, 4, 0)
lapply(df_FF5[,-1], function(x) {NWtest(x, 4, 0)})
