...
df_PB <- df_PB[,c("Dates",top_100_labels)]
...
#df_PB means the dataframe of shares' P/B ratios
names <- vector(mode =  "character", length = length(df_PB))
for(i in 1:length(df_PB)){
  names[i] <- sub(" .*$", "", names(df_PB[,-1])[i])
}
names <- names[-length(names)]

for(i in 1:length(names)){
  names[i] <- paste(names[i],"SJ", "Equity",sep = ".")
}

colnames(df_PB) <- c("Dates",names)

df_PB <- subset(df_PB, Dates %in% df_return$Dates)
df_PB[,-1] <- lapply(df_PB[,-1], as.numeric)
df_PB <- df_PB[ ,colnames(df_return[,-2])]
...
names_1 <- vector(mode =  "character", length = length(df_Size))
for(i in 1:length(df_Size)){
  names_1[i] <- sub(" .*$", "", names(df_Size[,-1])[i])
}
names_1 <- names_1[-length(names_1)]

for(i in 1:length(names_1)){
  names_1[i] <- paste(names_1[i],"SJ", "Equity",sep = ".")
}

colnames(df_Size) <- c("Dates",names_1)

df_Size <- subset(df_Size, Dates %in% df_return$Dates)
df_Size[,-1] <- as.data.frame(lapply(df_Size[,-1],as.numeric))
df_Size <- df_Size[ , colnames(df_return[,-2])]
...
#Divide the companies based on their Value.(P/B)
lookback_PB_list <- vector(mode = "list", length = Periods)
gap_PB_list <- vector(mode = "list", length = Periods)
holding_PB_list <- vector(mode = "list", length = Periods)

for (x in 1:Periods) { 
  lookback_PB_list[[x]] = df_PB[(1+((x-1)*Holding)):(Lookback+((x-1)*Holding)), ]
  gap_PB_list[[x]] = df_PB[(1+Lookback+(x-1)*Gap):(Lookback+Gap+(x-1)*Gap), ]
  holding_PB_list[[x]] = df_PB[(1+Lookback+Gap+(x-1)*Holding):(Lookback+Gap+Holding+(x-1)*Holding), ]
}
...
#### High PB: growth; Low PB: value
HML <- vector(mode = "list", length = Periods)

HML <- lapply(lookback_PB_list, function(x){
  
  PB_mean <- sapply(x[,-1], function(y){
    ans <- mean(unlist(y), na.rm = T)
    names(ans) <- names(y)
    return(ans)
  })
  
  order_PB <- order(PB_mean, na.last = NA) # order function will Perform on an ascending order
  order_PB <- PB_mean[order_PB]
  
  number_of_portfolios = 3
  N <- length(order_PB)
  quantile = trunc(N/number_of_portfolios)
  
  return(list(Value=names(order_PB)[1:quantile],
              Neutral=names(order_PB)[(quantile+1):(2*quantile)],
              Growth=names(order_PB)[(2*quantile+1):N]))
})
...
portfolio_df2 <- data.frame(Dates = df_return$Dates[-c(1:(Lookback+Gap))][1:(Periods*Holding)],
                            ...
                            B_Neutral_PB = unlist(Map(hld_period_calcs2, holding_return_list, HML, SMB,
                            ...
                            S_Neutral_PB = unlist(Map(hld_period_calcs2, holding_return_list, HML, SMB,
...
portfolio_clean2 <- portfolio_clean2 %>%
  mutate(Market_Level = rollapply((1+MR_RF),FUN = prod, width=1:nrow(portfolio_clean2),align="right"),
         SMB = round((1/3)*((1/3)*(S_Value+S_Neutral_PB+S_Growth)-
...
portfolio_df3 <- data.frame(Dates = df_return$Dates[-c(1:(Lookback+Gap))][1:(Periods*Holding)],
                            ...
                            B_Value = unlist(Map(hld_period_calcs3, holding_return_list, HML, SMB, 
                                                 port_number1 = 1, port_number2 = 2)),
                            B_Neutral_PB = unlist(Map(hld_period_calcs3, holding_return_list, HML, SMB,
                                                      port_number1 = 2, port_number2 = 2)),
                            ...
                            S_Value = unlist(Map(hld_period_calcs3, holding_return_list, HML, SMB,
                                                 port_number1 = 1, port_number2 = 1)),
                            S_Neutral_PB = unlist(Map(hld_period_calcs3, holding_return_list, HML, SMB,
                                                      port_number1 = 2, port_number2 = 1)),
...
portfolio_clean3 <- portfolio_clean3 %>%
  mutate(Market_Level = rollapply((1+MR_RF),FUN = prod, width=1:nrow(portfolio_clean3),align="right"),
         SMB = (1/3)*(S_Value+S_Neutral_PB+S_Growth)-(1/3)*(B_Value+B_Neutral_PB+B_Growth),
...
#Standardization: re-scale cross-sectional firm characteristics (PB,OP,INV)
#(x - mean(x)) / sd(x)
is.nan.data.frame <- function(x)
  do.call(cbind, lapply(x, is.nan))

df_std_PB <-  apply(t(df_PB[, -1]), 2, scale)
df_std_PB <- cbind(Dates,data.frame(t(df_std_PB)))
colnames(df_std_PB)<- colnames(df_PB)
...
hld_size_calcs <- function(a, b, c, port_number1, port_number2){
  ...
  port <- port %>% select_if(~!all(is.na(.))) %>% select_if(~!all(.==0))
  row_mean = rollapply(port, FUN = mean, width = 1:(Holding +1), na.rm = TRUE,align = "right",by.column = F)
  return(row_mean)
}
...
df_18_size <- data.frame(Dates = df_return$Dates[-c(1:(Lookback+Gap))][1:(Periods*Holding)],
                         B_Value = unlist(Map(hld_size_calcs, holding_size_list, HML, SMB, 
                                              port_number1 = 1, port_number2 = 2)),
                         B_Neutral_PB = unlist(Map(hld_size_calcs, holding_size_list, HML, SMB,
                                                   port_number1 = 2, port_number2 = 2)),
                         ...
                         S_Value = unlist(Map(hld_size_calcs, holding_size_list, HML, SMB,
                                              port_number1 = 1, port_number2 = 1)),
                         S_Neutral_PB = unlist(Map(hld_size_calcs, holding_size_list, HML, SMB,
                                                   port_number1 = 2, port_number2 = 1)),
...
df_18_std_size <- df_18_size
df_18_std_size[,-1] <- log(df_18_size[,-1])
df_18_std_size <-  apply(t(df_18_std_size[, -1]), MARGIN = 2, scale)
...
####df_18_PB####
hld_PB_calcs <- function(a, b, c, port_number1, port_number2){
  temp <- intersect(c[[port_number2]],b[[port_number1]])
  port <- a[,temp]
  port <- port %>% select_if(~!all(is.na(.))) %>% select_if(~!all(.==0))
  row_mean = rollapply(port, FUN = mean, width = 1:(Holding +1), na.rm = TRUE,align = "right",by.column = F)
...
df_18_PB <- data.frame(Dates = df_return$Dates[-c(1:(Lookback+Gap))][1:(Periods*Holding)],
                       B_Value = unlist(Map(hld_PB_calcs, holding_PB_list, HML, SMB, 
                                            port_number1 = 1, port_number2 = 2)),
                       B_Neutral_PB = unlist(Map(hld_PB_calcs, holding_PB_list, HML, SMB,
                                                 port_number1 = 2, port_number2 = 2)),
                       B_Growth = unlist(Map(hld_PB_calcs, holding_PB_list, HML, SMB,
                                             port_number1 = 3, port_number2 = 2)),
                       B_Robust = unlist(Map(hld_PB_calcs, holding_PB_list, RMW, SMB,
                                             port_number1 = 3, port_number2 = 2)),
                       B_Neutral_OP = unlist(Map(hld_PB_calcs, holding_PB_list, RMW, SMB,
                                                 port_number1 = 2, port_number2 = 2)),
                       B_Weak = unlist(Map(hld_PB_calcs, holding_PB_list, RMW, SMB,
                                           port_number1 = 1, port_number2 = 2)),
                       B_Conservative= unlist(Map(hld_PB_calcs, holding_PB_list, CMA, SMB,
                                                  port_number1 = 1, port_number2 = 2)),
                       B_Neutral_INV = unlist(Map(hld_PB_calcs, holding_PB_list, CMA, SMB,
                                                  port_number1 = 2, port_number2 = 2)),
                       B_Aggressive = unlist(Map(hld_PB_calcs, holding_PB_list, CMA, SMB,
                                                 port_number1 = 3, port_number2 = 2)),
                       S_Value = unlist(Map(hld_PB_calcs, holding_PB_list, HML, SMB,
                                            port_number1 = 1, port_number2 = 1)),
                       S_Neutral_PB = unlist(Map(hld_PB_calcs, holding_PB_list, HML, SMB,
                                                 port_number1 = 2, port_number2 = 1)),
                       S_Growth = unlist(Map(hld_PB_calcs, holding_PB_list, HML, SMB,
                                             port_number1 = 3, port_number2 = 1)),
                       S_Robust = unlist(Map(hld_PB_calcs, holding_PB_list, RMW, SMB,
                                             port_number1 = 3, port_number2 = 1)),
                       S_Neutral_OP = unlist(Map(hld_PB_calcs, holding_PB_list, RMW, SMB,
                                                 port_number1 = 2, port_number2 = 1)),
                       S_Weak = unlist(Map(hld_PB_calcs, holding_PB_list, RMW, SMB,
                                           port_number1 = 1, port_number2 = 1)),
                       S_Conservative= unlist(Map(hld_PB_calcs, holding_PB_list, CMA, SMB,
                                                  port_number1 = 1, port_number2 = 1)),
                       S_Neutral_INV = unlist(Map(hld_PB_calcs, holding_PB_list, CMA, SMB,
                                                  port_number1 = 2, port_number2 = 1)),
                       S_Aggressive = unlist(Map(hld_PB_calcs, holding_PB_list, CMA, SMB,
                                                 port_number1 = 3, port_number2 = 1))
)

count_nas <- sapply(df_18_PB, function(x){sum(is.na(x))})
count_nas

df_18_std_PB <-  apply(t(df_18_PB[, -1]), MARGIN = 2, scale)
df_18_std_PB <- cbind(Dates,data.frame(t(df_18_std_PB)))
colnames(df_18_std_PB) <- colnames(df_18_PR)
